...
class Ui_MainWindow(object):
    def __init__(self):
        ...
        self.photo = QtGui.QPixmap('youzhuan.gif')
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "有赚自动投注系统"))
...
class init_data(QThread):
    ...
    def remaxwrong(self):
        maxdb = p_mysql.MySQL()
        sql = 'select * from yz_js28 order by period DESC  limit 300'
    ...
    def q_curr_period(self):
        ...
        while True:
            ...
            try:
                ...
                if vote_info != None:
                    try:
                        temp_a = re.findall(r"parseInt\(\"(.+?)\"\)", vote_info[3].text)
                        temp_b = re.findall(r"jsNO = \"(.+?)\";", vote_info[3].text)
                ...
                if current_period != '':
                    ...
                    if firstflag_vote == '':
                        ...
                        config.read("Config_youzhuan.ini")
                        ...
                        try:
                            ...
                            config.write(open("Config_youzhuan.ini", "w"))
                    ...
                    if vote_list:  # 如果不为空，说明上一次投注了，判断是否正确。
                        try:
                            ...
                            sql = "select * from yz_js28 where period='" + vote_period + "' limit 1"
    ...
    def run(self):
        ...
        post_head = {"Accept": "application/json, text/javascript, */*",
                     ...
                     "Host": "www.youzhuan.com",
                     "Referer": "http://www.youzhuan.com/login.html",
                     "User-Agent": "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0)",
                     "X-Requested-With": "XMLHttpRequest"}
        try:
            # 产生一个0-1的随机数字 17位
            rankey = random.random()
            url = 'http://www.youzhuan.com/ajax.php'
            # 直接发送请求ajax 获得token
            # Post数据服务器
            req = requests.get(url, headers=post_head, allow_redirects=False)
            sub_ajax = {'act': 'login', 'key': rankey, 'tbUserAccount': self.var1, 'tbUserPwd': 'xfcctv1983', 'tfrr': 1}
            time.sleep(0.3)
